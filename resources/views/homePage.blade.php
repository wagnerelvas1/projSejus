          ...
          <div class="carousel-inner">
            @foreach($promocoes as $index => $jogo)
              <div class="carousel-item {{ $index == 0 ? 'active' : '' }}">
                <img src="{{ $jogo->imagem }}" class="d-block w-100" style="height: 400px; object-fit: cover;" alt="{{ $jogo->nome_jogo }}">
                <div class="carousel-caption d-none d-md-block bg-dark bg-opacity-50 rounded p-3">
                  <h5>{{ $jogo->nome_jogo }}</h5>
                  <p>
                    <span class="text-decoration-line-through text-muted">R$ {{ number_format($jogo->valor, 2, ',', '.') }}</span>
                    <span class="fw-bold text-warning ms-2">R$ {{ number_format($jogo->final_price, 2, ',', '.') }}</span>
                  </p>
                  <a href="{{ route('jogos.show', $jogo->id_jogo) }}" class="btn btn-primary">Ver promoção</a>
                </div>
              </div>
            @endforeach
          </div>
